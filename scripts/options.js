...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Função de importação modificada
    if (importConfigButton && importConfigFileInput) {
        importConfigButton.addEventListener('click', () => {
            ...
            reader.onload = async (event) => { // O handler do onload é async
                try {
                    const importedConfig = JSON.parse(event.target.result);
                    
                    // Validação básica da estrutura da configuração importada
                    ...
                    // --- ETAPA ADICIONADA: Limpar a configuração antiga do storage ---
                    displayStatus(importConfigStatus, "Limpando configuração antiga do armazenamento...", false, 20000); // Feedback para o usuário
                    await new Promise((resolve, reject) => {
                        chrome.storage.local.remove(CONFIG_STORAGE_KEY, () => {
                            if (chrome.runtime.lastError) {
                                // Se houver um erro ao remover, logamos mas podemos decidir prosseguir,
                                // já que o 'set' posterior deveria sobrescrever.
                                // Para um comportamento mais estrito, poderia dar um 'reject' aqui.
                                console.error(`Neuron (options.js): Erro ao tentar remover config antiga (${CONFIG_STORAGE_KEY}): ${chrome.runtime.lastError.message}`);
                                displayStatus(importConfigStatus, `Aviso: Não foi possível garantir a limpeza da config antiga (erro: ${chrome.runtime.lastError.message}). Tentando salvar a nova...`, true, 5000);
                            } else {
                                console.log(`Neuron (options.js): Configuração antiga (${CONFIG_STORAGE_KEY}) removida do storage (ou não existia).`);
                            }
                            resolve(); // Prossegue mesmo se houver um erro leve na remoção (o 'set' deve sobrescrever)
                        });
                    });
                    // --- FIM DA ETAPA ADICIONADA ---

                    currentFullConfig = importedConfig; // Define o objeto de configuração em memória para ser o importado
                    
                    // Salva o currentFullConfig (que agora é o importedConfig) no storage.
                    // Como a chave foi removida, isso cria a entrada do zero.
                    if (await saveFullConfig()) { // saveFullConfig usa chrome.storage.local.set
                        populateOptionsFromConfig(); // Recarrega toda a UI da página de opções com a nova configuração
                        ...
                        displayStatus(importConfigStatus, "Configurações importadas, mas houve um erro ao salvar no armazenamento.", true, 4000);
                    }
                } catch (error) {
                    console.error("Erro ao importar configuração:", error);
                    displayStatus(importConfigStatus, `Erro ao importar: ${error.message}`, true, 5000);
                } finally {
                    importConfigFileInput.value = ""; // Limpa o input de arquivo
                }
            };
            reader.onerror = () => {
                displayStatus(importConfigStatus, "Erro ao ler o arquivo selecionado.", true, 3000);
